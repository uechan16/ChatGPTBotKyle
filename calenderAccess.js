...
exports.getCalenderAsDay = async function (calendarId,day){
  const dateNow = new Date();
  const result = await calendar.events.list({
      calendarId: calendarId,
      timeMin: new Date(dateNow.getFullYear(),dateNow.getMonth(),dateNow.getDate() + day,7,0).toISOString(),
      maxResults: 20,
      singleEvents: true,
      orderBy: 'startTime',
      timeMax: new Date(dateNow.getFullYear(),dateNow.getMonth(),dateNow.getDate() + day,23,0).toISOString(),
  });
  //console.log(result.data.items.map((item) => console.log(item)));
  return result;
};
